...
class _CourseScreenState extends State<CourseScreen>
  ...
  bool isAdmin = false; // tracks Admin status
            ...
            content: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 10),
                  // Show URL field for Video or file picker for PDF.
            ...
            TextButton(
              onPressed: _isUpdating
                ...
                if (newTitle.isEmpty || newUrl.isEmpty) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                        content: Text('Please fill out both title and URL.')),
                  );
                  return;
                }
            ...
            TextButton(
              onPressed: () async {
                if (titleController.text.isNotEmpty &&
                  ...
                  // Optionally, refresh your course data here.
                } else {
                  ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget _buildOverview(Course course) {
    final String adminPhoneNumber = course.medium == 'Tamil'
        ? "+94757439885"
        : "+94766224999"; // Replace with actual admin numbers.
  ...
  void _showErrorSnackbar(String message) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(message), backgroundColor: Colors.redAccent, duration: Duration(seconds: 3)),
    );
  }
            ...
            ...section.videos.asMap().entries.map((entry) {
              ...
              return ListTile(
                leading: Icon(
                  ...
                  color: isFirstVideo ? Colors.green : (canAccessVideo ? Colors.blueAccent : Colors.redAccent),
                ),
                title: Text(video.title ?? 'Untitled Video'),
                subtitle: Text(isFirstVideo ? 'Free Preview' : (canAccessVideo ? 'Available to Play' : 'Enroll to Play')),
                onTap: () {
                  if (canAccessVideo) {
  ...
  void _showAddFeedbackDialog(BuildContext context, String courseId) {
    ...
    final TextEditingController _localFeedbackController = TextEditingController();
    double _localRating = 3.0;
      ...
      builder: (context) => AlertDialog(
        ...
        content: Form(
          ...
          child: Column(
            ...
            children: [
              TextFormField(
                controller: _localFeedbackController,
                decoration: const InputDecoration(
              ...
              const SizedBox(height: 12),
              Text('Rate this course:', style: TextStyle(fontSize: 16, color: Colors.grey[700])),
              const SizedBox(height: 5),
              RatingBar.builder(
          ...
          TextButton(
            onPressed: () async {
              if (_formKey.currentState!.validate()) {
                ...
                bool alreadySubmitted = _course!.feedbacks.any((fb) => fb.userId.toString() == currentUser.id);
                if (alreadySubmitted) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                ...
                final result = await _addFeedback(_course!.id!, currentUser.name ?? 'Anonymous', _localFeedbackController.text, currentUser.id!, _localRating);
                _localFeedbackController.clear();
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text(result ? 'Feedback submitted successfully!' : 'Failed to submit feedback.'),
            ...
            child: _isSubmitting
                ? const SpinKitDoubleBounce(color: Colors.white, size: 20.0)
                : const Text('Submit'),
          ),
        ],
      ),
  ...
  Future<bool> _addFeedback(String courseId, String userName, String feedbackText, String userId, double rating) async {
    try {
      final courseProvider = Provider.of<CourseProvider>(context, listen: false);
    ...
    return FutureBuilder<CustomUser?>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return const Center(child: SpinKitDoubleBounce(color: Colors.blueAccent));
    ...
    return FutureBuilder<DocumentSnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        if (snapshot.hasError) {
          return const Padding(padding: EdgeInsets.all(8.0), child: Center(child: Text('Error fetching user')));
        }
        if (!snapshot.hasData || snapshot.data == null) {
          return const Padding(padding: EdgeInsets.all(8.0), child: Center(child: Text('User data not available')));
        }
        ...
        final formattedDate = DateFormat('yMMMd').format(feedback.date!.toDate());
        ...
        return Card(
          ...
          elevation: 3,
          color: isCurrentUserFeedback ? Colors.lightBlue.shade50 : Colors.white,
          child: Column(
            children: [
              ListTile(
                leading: CachedNetworkImage(
                  ...
                  placeholder: (context, url) => const CircleAvatar(
                    ...
                    child: SpinKitDoubleBounce(color: Colors.blueAccent, size: 20.0),
                  ),
                ...
                title: Row(
                  children: [
                    ...
                    if (feedback.rating != null)
                      ...
                      const Text('No Rating', style: TextStyle(fontSize: 12, color: Colors.grey)),
                  ],
                ),
            ...
            TextButton(
              onPressed: () async {
                if (_feedbackController.text.isNotEmpty) {
                  ...
                  try {
                    await Provider.of<CourseProvider>(context, listen: false)
                        .updateFeedback(_course!.id!, feedback.userId.toString(), _feedbackController.text, _localRating);
                    setState(() {
                      _isSubmitting = false;
                    });
                    Navigator.pop(context);
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Feedback updated successfully!'), backgroundColor: Colors.green),
                    );
                    await _initializeCourse();
                  } catch (e) {
                    setState(() {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(content: Text('Failed to update feedback: $e'), backgroundColor: Colors.red),
                    );
                  }
                } else {
                  ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _deleteFeedback(String courseId, FeedBack feedback) async {
    ...
    if (shouldDelete == true) {
      ...
      try {
        await Provider.of<CourseProvider>(context, listen: false).deleteFeedback(_course!.id!, feedback.userId.toString());
        setState(() {
          _isSubmitting = false;
        });
        await _initializeCourse();
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Feedback deleted successfully.'), backgroundColor: Colors.green),
        );
      } catch (e) {
        setState(() {
          _isSubmitting = false;
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Failed to delete feedback. Please try again.'), backgroundColor: Colors.red),
        );
      }
    }
  }
            ...
            ElevatedButton.icon(
              ...
              label: _isSubmitting
                ...
                child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2.0),
                  ...
                  : Text('Submit Feedback', style: TextStyle(fontSize: 16, color: Colors.white)),
            )
          ...
          return Scaffold(
            ...
            body: Column(
              children: [
                ...
                else if (_course!.sections.isNotEmpty &&
                    _course!.sections.first.videos.isNotEmpty)
                  ClipRRect(
                    borderRadius: BorderRadius.circular(16.0),
                    child: AspectRatio(
                      ...
                      child: YoutubePlayer(
                        controller: YoutubePlayerController(
                          initialVideoId: YoutubePlayer.convertUrlToId(_course!.sections.first.videos.first.videoUrl!) ?? '',
                          flags: const YoutubePlayerFlags(
                            autoPlay: false,
                            mute: false,
                            enableCaption: true,
